            ...
            <table id="example" class="table table-striped responsive-utilities jambo_table">
                ...
                <tbody>
                    ...
                    <tr class="{{ $count % 2 === 0 ? 'even' : 'odd'}} pointer">
                        ...
                        <td class=" ">{{ $data->categories->title }}</td>
                        <td class=" ">{{ $data->categories->description }}</td>
...
<script type="text/javascript">
    $(function(){
        $("#btnAdd").click(function(){
            $(".modal-body").load('{{route('manage.categoriesAuthAdd')}}/{{$user->id}}',function(){
        ...
        $(".btnDel").click(function(){
            if(confirm("確定要刪除授權嗎?")){
                $.ajax({
                      method: "get",
                      url: "{{ route('manage.categoriesAuthDel') }}",
